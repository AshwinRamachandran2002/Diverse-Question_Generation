...
async function main() {
  ...
  for (let i=0; i < 5; i++) {
    ...
    prompt = 'Give me '+ noFAQs +' FAqs for the following description from cutomers perspective. ' + employee.default[i].desc + '. just a simple list of questions, no need of answers or additional text'
  ...
  var json = JSON.stringify(employee.default);
  fs.writeFile('../469_metadata_chatgpt.json', json, function(err) {
    if (err) throw err;
