...
#  { pd3category: { ... } }]
answer = []
with gzip.open('All_Amazon_Meta.json.gz', 'rb') as f:
    line = f.readline()
    products = json.loads(line)
# data = json.load(open('sample_dirty_4.json'))
    ...
    if products["tech1"] != None:
        if products["tech1"] != "":
            ...
            th_selectors = response.xpath(
                "//th/text()").extract()  # [0].strip()
            td_selectors = response.xpath(
                "//td/text()").extract()  # [0].strip()
            dict_pdt = {}
            for i in range(len(th_selectors)):
                dict_pdt[th_selectors[i].strip()] = td_selectors[i].strip()
...
json_bytes = obj.encode('utf-8')            # 3. bytes (i.e. UTF-8)

with gzip.open("clean_data.json.gz", 'w') as fout:       # 4. fewer bytes (i.e. gzip)
